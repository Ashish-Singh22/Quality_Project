...
const cnFindController = async (req, res) => {
  try {
    ...
    const results = await cnModel.find({
      $or: queryConditions
    });

    return res.status(200).json({
      message: "Filtered cn data fetched successfully",
      success: true,
      error: false,
      data: results,
    });
  } catch (error) {
    console.error("Error fetching cn records:", error);
...
module.exports = cnFindController;
