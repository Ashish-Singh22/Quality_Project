...
const ctFindController = async (req, res) => {
  try {
    ...
    const results = await ctModel.find({
      $or: queryConditions
    });

    return res.status(200).json({
      message: "Filtered ct data fetched successfully",
      success: true,
      error: false,
      data: results,
    });
  } catch (error) {
    console.error("Error fetching ct records:", error);
...
module.exports = ctFindController;
