...
const clFindController = async (req, res) => {
  try {
    ...
    const results = await clModel.find({
      $or: queryConditions
    });

    return res.status(200).json({
      message: "Filtered cl data fetched successfully",
      success: true,
      error: false,
      data: results,
    });
  } catch (error) {
    console.error("Error fetching cl records:", error);
...
module.exports = clFindController;
