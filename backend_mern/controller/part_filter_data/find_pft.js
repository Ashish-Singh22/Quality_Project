...
const pftFindController = async (req, res) => {
  try {
    ...
    const results = await pftModel.find({
      $or: queryConditions
    });

    return res.status(200).json({
      message: "Filtered pft data fetched successfully",
      success: true,
      error: false,
      data: results,
    });
  } catch (error) {
    console.error("Error fetching pft records:", error);
...
module.exports = pftFindController;
